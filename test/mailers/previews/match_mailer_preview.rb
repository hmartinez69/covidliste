...
class MatchMailerPreview < ActionMailer::Preview
  ...
  def match_confirmation_instructions_multi_match
    match = Match.last
    FactoryBot.create(:match_multi, :confirmed, user: match.user, campaign: match.campaign)
    FactoryBot.create(:match_multi, :available, user: match.user, campaign: match.campaign)
    MatchMailer.with(match: match).match_confirmation_instructions.deliver_now
  end
